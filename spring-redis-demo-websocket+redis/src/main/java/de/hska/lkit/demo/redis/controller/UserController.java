...
import de.hska.lkit.demo.redis.model.PostSubscriber;
...
public class UserController {
	...
	@Autowired
	private MessageListenerAdapter messageLiAdap;
...
//		this.sessionRepo = sessionRepo;
	...
	public void newPostBySocket(Post post)throws Exception {
		...
		this.reTemplate.convertAndSend("/topic/chat-room/"+post.getAuthorId(), post);		
	   // Thread.sleep(500);//der redis container braucht bissle zeit bis er die nachricht von redischannel abfängt, wenn ich das nicht mache  wird simpleTemplate.convertandsend sofort ausgeführt aber in den subscriber wurde noch nix gespeichert
	    ...
	    //System.out.println("The subscriber ffrom redis get: " +subs.getMSG().getContent());
	    //System.out.println("I am inside newPostBySocket and getting Post from messageListener, which get Post from Redis: "+ subscriber.getPost().getAuthorId()+"  "+subscriber.getPost().getContent());
	    
		//this.simpleMessTemplate.convertAndSend("/topic/chat-room", subscriber.getPost());
	   // this.simpleMessTemplate.convertAndSend("/topic/chat-room", subs.getMSG());
	...
	public String gotoHome(@ModelAttribute User user, HttpServletRequest request, Model model, @ModelAttribute Post post,@RequestParam(value="startSearching", required=false) String searchPatternn,  @RequestParam(value="spattern" ,required = false) String searchPattern,@ModelAttribute("spattern") String searchPattern3) {
		...
		//teil für redis post channel subscribe
		Set<String> userIFollow= userRepo.getPeopleIFollow(user.getUsername());
		container.addMessageListener(messageLiAdap, new PatternTopic("topic/chat-room/"+user.getUsername()));
		for (String s : userIFollow) {
		    container.addMessageListener(messageLiAdap, new PatternTopic("topic/chat-room/"+s));
		    System.out.println("Forschleife für addMessageListener: "+s);
		}
