...
public class RedisRepository {
	...
	public String addAuth(String uname, long timeout, TimeUnit tUnit) {
		...
		template.opsForValue().set("auth:" + auth , uname, timeout, tUnit);
		return auth;
	}
	...
	public void deleteAuth(String uname) {
		...
		template.opsForHash().delete(authKey, "auth");
		//template.delete("auth");
		List<String> keysToDelete = Arrays.asList( "auth:" + auth );
		
//		List<String> keysToDelete = Arrays.asList(authKey, "auth:" + auth );
